...
flowchart TD
    ...
    APP --> SLOT{"tagger 差し替え層<br/>predict と fit さえあればOK"}
    SLOT --> A["PerceptronTagger<br/>今の参照実装・純JDK<br/>しくみが全部見える"]
    SLOT --> B["CRF<br/>例: MALLET<br/>もっと精度↑"]
    SLOT --> C["文字BERT → ONNX<br/>onnxruntime で推論<br/>さらに精度↑↑"]
    style A fill:#d3f9d8
```

**みどり先生**：将来、もっと強い切り方が欲しくなったら——
- **MALLET（マレット）の CRF**（条件付き確率場）に替える。CRF は構造化パーセプトロンの「確率版の親戚」みたいなものだ。
- あるいは **文字 BERT（バート）**みたいな大きいモデルを学習して、**ONNX（オニックス）形式**に変換し、**onnxruntime（オニックスランタイム）**で推論する。

**ツムギ**：でも、そんなに中身が変わったら、`parse` の使い方も変わっちゃうんじゃ……？

**みどり先生**：そこが美しいところ。**変わらない**んだ。
`tagger` が `predict`（文字列→タグ）と `fit`（学習）さえ提供すれば、`AddressParser` から見れば中身が何であろうと同じ。だから——

**みどり先生**：**インターフェース（窓口）は変えずに、中身だけどんどん強くできる**。
...
**みどり先生**：本編は、これでおしまい。でも、君たちの旅はここからが本番だ。

**ツムギ**：本番……？

**みどり先生**：このコースを読み終えた君は、もう **kugiri のメンテナー**だよ。
全ソースが「なぜそう書いてあるか」説明できる。設計の全体像は [`../DESIGN.md`](../DESIGN.md) に、
次にやる作業（実データ取込・self-training・CRF/BERT 差し替えなど）は [`../HANDOFF.md`](../HANDOFF.md) に書いてある。

**ゲンタ**：差し替え層の話、さっそく HANDOFF に「CRF/BERT 差し替え」って書いてあるじゃん。やってみるか。

**アザミ**：……次に困っている「ラベルのない子」がいたら、今度はあなたが助けてあげてね。

**みどり先生**：道具のおさらいが必要なら、付録へ。まずは「行列のきほん」からどうぞ。
あわてない、あわてない。——よくここまで来た。おつかれさま。

[← 第18章](18-synth-data.md) ・ [付録A2 →](A2-gyouretsu.md)
