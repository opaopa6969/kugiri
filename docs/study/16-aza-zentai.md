...
**みどり先生**：1行目の3つの数字は、第11章でやった **P / R / F1** だ。ここでは「**字の内部の切れ目（区切り位置）**」が当たったかで測っている。
...
**ツムギ**：じゃあ、もっと良くできるかもってこと？

**みどり先生**：そう。チューニングや本番データへの差し替えは **第19章** と、開発メモの `docs/HANDOFF.md` に続く。今日は「**正解ゼロでも、ここまで来られる**」を確かめられた。それで十分すぎる成果だよ。

> 📌 注意：このデモは **合成データ**（規則的に作った住所）だ。第18章でやるように、合成の数字は本物の実力そのものではない。本当の評価は、本物データの hold-out（取り置き）でやる。それでも「教師なしで字が当たる」という**仕組みの正しさ**は、ここでしっかり見えている。
...
- 結果：**正解ラベルゼロ**で `P=1.000 / R=0.708 / F1=0.829`、字スパン完全一致 `353/400=0.883`。「**教えてもらえなくても、データのかたよりから当てられる**」——これが教師なし学習。
- ただし完璧ではない（R=0.708、慎重すぎて取りこぼし）。改善余地は第19章・`HANDOFF.md` へ。
