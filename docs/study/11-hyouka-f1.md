...
## 伏線回収：entity（スパン）単位の評価は、もう実装されている

**ツムギ**：さっき「token単位とは別に entity単位がある」って言ってましたよね。それって作ったんですか？

**みどり先生**：作ったよ。`eval.SpanEval` と `AddressParser.evaluateSpans` がそれだ。
BIOES タグ列を **(ラベル, 開始, 終了) のスパン**に戻して、**ラベルも範囲もぴったり一致**したものだけを
正解(tp)に数える。「盛岡市」を `市[0,3)` と1個のかたまりで見るんだ。

```java
// eval.SpanEval.spans: BIOESタグ列 -> スパン集合（B/S で開始、E/S で確定）
// AddressParser.evaluateSpans: per-label / micro / macro のスパン P/R/F1 ＋ 混同行列
```

**ゲンタ**：token単位と何がちがうの？

**みどり先生**：たとえば「1234番地」を `123|4番地` と切ってしまうと、token単位なら文字の大半は当たって
高得点に見える。でも **スパン単位なら「地番ぴったり」ではないので tp=0**（fp と fn が1ずつ）。
*かたまりで正しいか* を厳しく見るのが entity 単位だよ。

さらに、**混同行列（こんどうぎょうれつ）**＝「どのラベルを、どのラベルと取り違えたか」の集計も出る。
たとえば `棟 → 方書き : 3`（建物名を方書きと間違えた3件）みたいに、**弱点が名指し**で分かる。

### hold-out も実装ずみ

**みどり先生**：そして次の節の「取り分けデータ」も、`AddressParser.holdout(data, 0.8, seed)` で
8:2 に分けられる。`demo/EvalDemo` がこれ一式（hold-out → 学習 → スパン評価＋混同）をやって見せる。

```bash
mvn -q exec:java -Dexec.mainClass=org.unlaxer.kugiri.demo.EvalDemo -Dstdout.encoding=UTF-8
```

出てくるのは per-label のスパン P/R/F1、micro/macro、そして混同 top。
合成データなので数字は高いが、`丁目 -> 地番 : 1` のような**実際の取り違え**もちゃんと顔を出す。

> 📌 token単位（`evaluate`）と entity単位（`evaluateSpans`）は**両方ある**。前者は学習の進み具合の
> ざっくり把握、後者が「部品として正しく取れたか」の本番指標。kugiri では entity スパン F1 を重視する。
...
- **かたまり（entity）単位**のスパン P/R/F1・混同行列は `eval.SpanEval` / `AddressParser.evaluateSpans`、
  hold-out は `AddressParser.holdout`、まとめて動かすのが `demo/EvalDemo`（すべて実装済み）。
